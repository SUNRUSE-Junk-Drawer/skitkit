...
describe(`withdrawProposal`, () => {
  describe(`imports`, () => {
    let withdrawProposal: { __get__(name: string): unknown };

    beforeAll(() => {
      withdrawProposal = rewire(`.`);
    });

    it(`histories`, () => {
      expect(withdrawProposal.__get__(`importedHistories`)).toBe(histories);
    });

    it(`refresh`, () => {
      expect(withdrawProposal.__get__(`importedRefresh`)).toBe(refresh);
    });
  });
  ...
  describe(`on execution`, () => {
    ...
    beforeAll(() => {
      ...
      withdrawProposal.__set__(`importedHistories`, {
      ...
      withdrawProposal.__set__(`importedRefresh`, refresh);

      withdrawProposal.__get__(`withdrawProposal`)(`Test Skit Uuid`);
    });
